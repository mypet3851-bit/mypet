    ...
    try {
      ...
      for (const job of due) {
        try {
          ...
          const req = { body: { title: job.title, body: job.body, data: job.data, badge: job.badge, sound: job.sound, channelId: job?.audience?.channelId, userId: job?.audience?.userId, email: job?.audience?.email }, user: { _id: job.createdBy } };
